...
function! giit#component#browse#process#execute(git, args) abort
  let scheme    = a:args.get('-s|--scheme', '_')
  let commit    = a:args.get_p(1, '')
  let filename  = a:args.get_p(2, '')
  let selection = giit#util#selection#parse(a:args.get('--selection', ''))
  let exact     = a:args.get('-e|--exact')
...
function! s:find_url(git, scheme, commit, filename, selection, exact) abort
  ...
  let url = giit#component#browse#formatter#format(a:scheme, remote_url, {
        \ 'path': relpath,
        \ 'rev1': rev1,
        \ 'rev2': rev2,
        \ 'revision1': revision1,
